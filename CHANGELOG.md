...
- A `[logging]` section. A packet that fails to translate is always reported in the console, naming the packet, the stage it failed at, the peer and the two protocol versions; `max-error-length` caps how much of that is written, and `0` keeps all of it. `log-other-conversion-warnings` additionally warns about a packet a transform gives up on, and `log-blocked-joins` reports every refused join.
