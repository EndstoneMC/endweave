...
- **Cancellation reaches only as far as the schema does.** `has_packet` means "bedrock-protocol
  ...
  `SetPlayerFurnaceOptions` (351) is the live case — modelled at 2192 alone, so leaving 2192 cancels
  it rather than handing an older peer an id it never knew. `RecordStarted` (352) is the other half:
  2192 names the id but no version models the packet, so it reads false at both ends and is relayed
...
- **A declaration only reaches the endpoints.** `handle` decodes at `From` and encodes at `To`, so a
  ...
  because 2192 leaves both unreshaped, so `packet_of<2192, Id>` is the same type as
...
- **One file per source version,** `protocols/<version>/`, holding every specialization whose source
  ...
  included, so one file answers everything about that era's packets. v1001 and v2192 are the
