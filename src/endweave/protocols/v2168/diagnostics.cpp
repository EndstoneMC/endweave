...
void Transformer<bp::MemoryCategoryCounter_<2168>, bp::MemoryCategoryCounter_<2187>>::transform(
    Context<bp::MemoryCategoryCounter_<2187>> &ctx, bp::MemoryCategoryCounter_<2168> &&from)
{
    auto &to = ctx.out();
    // ENDWEAVE: 2187 dropped Persona_Textures and shifted every category above it down one, so the name
    // carries the meaning. The dropped one has no 2187 counterpart and lands on Unknown.
    to.category = bp::enum_cast<bp::MemoryCategory_<2187>>(bp::enum_name(from.category))
                      .value_or(bp::MemoryCategory_<2187>::UNKNOWN);
    to.current_bytes = from.current_bytes;
}

void Transformer<bp::EntityDiagnosticTimingInfo_<2168>, bp::EntityDiagnosticTimingInfo_<2187>>::transform(
    Context<bp::EntityDiagnosticTimingInfo_<2187>> &ctx, bp::EntityDiagnosticTimingInfo_<2168> &&from)
{
    auto &to = ctx.out();
    ...
    // ENDWEAVE: 2168 locates the timed entity by nothing but its name, and absent is what 2187 writes
    // when the client has no position to offer.
    to.position = std::nullopt;
    to.dimension = std::nullopt;
}

void Transformer<bp::ServerboundDiagnosticsPacket_<2168>, bp::ServerboundDiagnosticsPacket_<2187>>::transform(
    Context<bp::ServerboundDiagnosticsPacket_<2187>> &ctx, bp::ServerboundDiagnosticsPacket_<2168> &&from)
{
    auto &to = ctx.out();
    to.avg_fps = from.avg_fps;
    to.avg_server_sim_tick_time_ms = from.avg_server_sim_tick_time_ms;
