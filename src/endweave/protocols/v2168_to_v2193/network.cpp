...
namespace endweave {
void Transformer<bp::DisconnectPacket_<2168>, bp::DisconnectPacket_<2193>>::transform(
    Context<bp::DisconnectPacket_<2193>> &ctx, bp::DisconnectPacket_<2168> &&from)
{
    using Reason = bp::DisconnectFailReason_<2193>;
    auto &to = ctx.out();
    // ENDWEAVE: 2193 inserted MissingStructureData and UnsupportedTransport ahead of the trailing
...
void Transformer<bp::PacketViolationWarningPacket_<2168>, bp::PacketViolationWarningPacket_<2193>>::transform(
    Context<bp::PacketViolationWarningPacket_<2193>> &ctx, bp::PacketViolationWarningPacket_<2168> &&from)
{
    using Ids = bp::MinecraftPacketIds_<2193>;
    auto &to = ctx.out();
