...
namespace endweave {
void Transformer<bp::TextDataPayload_<2168>, bp::TextDataPayload_<2193>>::transform(
    Context<bp::TextDataPayload_<2193>> &ctx, bp::TextDataPayload_<2168> &&from)
{
    auto &to = ctx.out();
    to.text = std::move(from.text);
    to.use_rotation = from.use_rotation;
    to.background_color = from.background_color;
    // ENDWEAVE: 2168 sends no line gap, so multi-line text reaches a 2193 client with its lines
