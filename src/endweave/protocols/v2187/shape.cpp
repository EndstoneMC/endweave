#include "endweave/protocols/v2187/shape.h"
...
void Transformer<bp::TextDataPayload_<2187>, bp::TextDataPayload_<2168>>::transform(
    Context<bp::TextDataPayload_<2168>> &ctx, bp::TextDataPayload_<2187> &&from)
{
    auto &to = ctx.out();
...
void Transformer<bp::PrimitiveShapeDataPayload_<2187>, bp::PrimitiveShapeDataPayload_<2168>>::transform(
    Context<bp::PrimitiveShapeDataPayload_<2168>> &ctx, bp::PrimitiveShapeDataPayload_<2187> &&from)
{
    auto &to = ctx.out();
    ...
    std::visit(
        [&to, &ctx](auto &alt) {
            if constexpr (std::is_same_v<std::remove_cvref_t<decltype(alt)>, bp::TextDataPayload_<2187>>) {
...
void Transformer<bp::PrimitiveShapesPacket_<2187>, bp::PrimitiveShapesPacket_<2168>>::transform(
    Context<bp::PrimitiveShapesPacket_<2168>> &ctx, bp::PrimitiveShapesPacket_<2187> &&from)
{
    auto &to = ctx.out();
