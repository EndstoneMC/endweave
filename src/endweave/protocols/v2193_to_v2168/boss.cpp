...
void Transformer<bp::BossEventPacket_<2193>, bp::BossEventPacket_<2168>>::transform(
    Context<bp::BossEventPacket_<2168>> &ctx, bp::BossEventPacket_<2193> &&from)
{
    auto &to = ctx.out();
    to.boss_id = from.boss_id;
    // ENDWEAVE: player_id has no source at 2193, which dropped it, so it is invented as the
