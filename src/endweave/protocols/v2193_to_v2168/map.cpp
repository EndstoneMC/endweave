...
void Transformer<bp::MapDecoration_<2193>, bp::MapDecoration_<2168>>::transform(Context<bp::MapDecoration_<2168>> &ctx,
                                                                                bp::MapDecoration_<2193> &&from)
{
    using Type = bp::MapDecoration_<2168>::Type;
    using NewType = bp::MapDecoration_<2193>::Type;
    auto &to = ctx.out();
    // ENDWEAVE: the five markers 2193 added have no 2168 image, so each borrows the closest-looking one.
    switch (from.image) {
