...
void Transformer<bp::DisconnectPacket_<2193>, bp::DisconnectPacket_<2168>>::transform(
    Context<bp::DisconnectPacket_<2168>> &ctx, bp::DisconnectPacket_<2193> &&from)
{
    using Reason = bp::DisconnectFailReason_<2168>;
    auto &to = ctx.out();
    // ENDWEAVE: MissingStructureData and UnsupportedTransport are 2193's alone and have no 2168 reason,
...
void Transformer<bp::PacketViolationWarningPacket_<2193>, bp::PacketViolationWarningPacket_<2168>>::transform(
    Context<bp::PacketViolationWarningPacket_<2168>> &ctx, bp::PacketViolationWarningPacket_<2193> &&from)
{
    using Ids = bp::MinecraftPacketIds_<2168>;
    auto &to = ctx.out();
    to.violation_type = from.violation_type;
    to.violation_severity = from.violation_severity;
    // ENDWEAVE: SetPlayerFurnaceOptions and RecordStarted are 2193's alone, so a complaint about either
