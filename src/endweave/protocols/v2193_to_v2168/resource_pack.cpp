...
void Transformer<bp::ServerboundPackSettingChangePacket_<2193>, bp::ServerboundPackSettingChangePacket_<2168>>::
    transform(Context<bp::ServerboundPackSettingChangePacket_<2168>> &ctx,
              bp::ServerboundPackSettingChangePacket_<2193> &&from)
{
    auto &to = ctx.out();
    to.pack_id = from.pack_id;
    to.pack_setting_name = std::move(from.pack_setting_name);
    // ENDWEAVE: 2193 added a list of strings as a fourth setting shape, and 2168 has no arm to put it
