...
def _build_v924_start_game(has_server_join_info: bool, has_gathering: bool = False) -> bytes:
    ...
    if has_server_join_info:
        ...
        if has_gathering:
            for s in ["exp-id", "exp-name", "exp-world-id",
                       "exp-world-name", "creator-id", "store-id", "store-name"]:
                w.write_string(s)
...
class TestRewriteStartGame:
    ...
    def test_join_info_true_no_gathering(self):
        ...
        payload = _build_v924_start_game(
        ...
        wrapper_base = PacketWrapper(
            _build_v924_start_game(has_server_join_info=False)
        )
